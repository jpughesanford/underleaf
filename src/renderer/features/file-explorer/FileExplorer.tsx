...
export default function FileExplorer({ projectPath, activeFile, onOpenFile, mainDoc, detectedMainDoc, onSetMainDoc }: Props) {
  ...
  const refresh = useCallback(async () => {
    setLoading(true)
    try {
      const nodes = await window.api.files.tree(projectPath)
      setTree(nodes)
    } finally {
      setLoading(false)
    }
  ...
  async function handleCreate(parentPath: string, type: 'file' | 'folder', name: string) {
    ...
    try {
      if (type === 'folder') {
        ...
        await window.api.files.write(fullPath, '')
        onOpenFile(fullPath)
      }
      await refresh()
    } catch (e) {
  ...
  async function handleDrop(targetDirPath: string, e: React.DragEvent) {
    ...
    if (e.dataTransfer.files.length > 0) {
      ...
      for (const file of files) {
        const filePath = window.api.files.getPathFor(file)
        if (!filePath) continue
        const destPath = `${targetDirPath}/${file.name}`
        try {
          await window.api.files.copy(filePath, destPath)
        } catch (err) {
    ...
    if (srcPath && srcPath !== targetDirPath) {
      ...
      try {
        await window.api.files.rename(srcPath, destPath)
        await refresh()
      } catch (err) {
