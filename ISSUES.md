...
- [x] the menu bar needs an overall. Right now "close window" is under file. Is this right? If so thats fine, i am just checking. There shouold also be a "new window" option. Is there anything else that should go there that I am not thinking of. 
- [x] is the save button really useful? Most folks use cmd+s these days. Howeer, there needs to be some signal for the user, like in VS code, that the file has changed from its saved version.  
- [ ] The home page needs more options for the user. It should be possible to "fetch all" in one click. It should also be possible to right click and fetch or push a single repo. It should be possible to delete a repo. Recall that conflicts might emerge and so it might be required to upon a conflict resolution session. I would like the give the user the option to replacing the existing repo with remote, in case there current changes arent useful and they dont want to bother with conflict resolution. 
...
- [x] Allow users to drag files into the file explorer. behind the scenes, this copies the dragged file into the folder they hover over
- [x] should be able add new file, add new folder, and drag files around in the file explorer. 
